	                	...
	                	<div class="row">
	                		<aside class="col-md-4 col-lg-3">
	                			<ul class="nav nav-dashboard flex-column mb-3 mb-md-0" role="tablist">
								            ...
								            <li class="nav-item">
								                <a class="nav-link" @click="activateTab('tab-orders-link')" :class="{ 'active': activeTab === 'tab-orders-link' }">Ordini</a>
								            </li>
								            <li class="nav-item">
								                <a class="nav-link" @click="activateTab('tab-address-link')" :class="{ 'active': activeTab === 'tab-address-link' }">Indirizzi</a>
								            </li>
								            <li class="nav-item">
								                <a class="nav-link" @click="activateTab('tab-account-link')" :class="{ 'active': activeTab === 'tab-account-link' }">Dettagli Account</a>
								            </li>
								            <li class="nav-item">
	                		...
	                		<div class="col-md-8 col-lg-9">
	                			<div class="tab-content">
								          <div :class="{ 'tab-pane': true, 'fade': true, 'show': activeTab === 'tab-dashboard-link', 'active': activeTab === 'tab-dashboard-link' }" id="tab-dashboard-link" role="tabpanel">
								    	      <p>Ciao <span class="font-weight-normal text-dark">{{ user.firstName }}</span> (Non sei tu? <a href="#" @click.prevent="logout">Log out</a>) 
								    	      <br>
								    	      Dalla tua dashboard puoi visionare i tuoi <a href="#tab-orders" class="tab-trigger-link link-underline">ordini</a>, puoi gestire gli <a href="#tab-address" class="tab-trigger-link">indirizzi di spedizione</a>, e <a href="#tab-account" class="tab-trigger-link">modificare i dati del tuo account</a>.</p>
								          </div><!-- .End .tab-pane -->
								          ...
								          <div :class="{ 'tab-pane': true, 'fade': true, 'show': activeTab === 'tab-orders-link', 'active': activeTab === 'tab-orders-link' }" id="tab-orders-link" role="tabpanel">
								    	      <p v-if="!userOrders">Non hai effetuato nessun ordine.</p>
                            <div v-else class="row">
								    		      <div class="col-lg-7">
								    		      	<div class="card card-dashboard" v-for="order in userOrders" :key="order.id">
								    		      		<div class="card-body">
								    		      			<h3 class="card-title">{{ order.id }}</h3><!-- End .card-title -->
                                  
												      		  <p>Data ordine: {{ formatTimestamp(order.createdAt) }}<br>
                                    Totale: {{ order.total.toFixed(2) }} €</p>
                                    <p v-for="detail in order.orderDetails" :key="detail.id">
                                    Prodotto ordinato: {{ detail.title }} ( x {{ detail.quantity }} )<br>
                                    </p>
								    		      		</div><!-- End .card-body -->
								    		      	...
								    		      	<div class="card card-dashboard">
								    		      		<div class="card-body">
								    		      			<h3 class="card-title">Billing Address</h3><!-- End .card-title -->
												      		  ...
												      		  {{ user.lastName }}<br>
												      		  {{ address.street }}<br>
												      		  {{ address.city }}, {{ address.district }}, {{ address.postalCode }}<br>
												      		  {{ address.phoneNumber }}<br>
                                    {{ user.email }}<br>
												      		  <a href="#">Modifica <i class="icon-edit"></i></a>
                                    </p>
								    		      		</div><!-- End .card-body -->
								    		      	</div><!-- End .card-dashboard -->
								    		      </div><!-- End .col-lg-6 -->
